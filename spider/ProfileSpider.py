...
class ProfileSpider(Spider):
    ...
    def extractInfo(self,content):
        ...
        tree=etree.HTML(content)
        #提取微博vip等级
        vipLevel=tree.xpath('/html/body/div['+str(self.placeNum+1)+']/text()')[0][5:]
        if ('未开' in vipLevel): 
            vipLevel='null'
        self.obj['vipLevel']=vipLevel
        logging.debug('会员等级:'+vipLevel)
        #初始化变量
        name='null'
        location='null'
        gender='null'
        ...
        logging.debug('birthday:'+birthday)
        self.obj['intro']=intro
        logging.debug('intro:'+intro)
        self.obj['identifyContent']=identifyContent
        logging.debug('identifyContent:'+identifyContent)
